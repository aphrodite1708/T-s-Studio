    ...
    <div className="studio-page">
      <div className="studio-sidebar">
        ...
        <div className="chip-group">
          {GENRE_CHIPS.map((c) => (
            <button key={c} className="chip" onClick={() => chipSend(c)} disabled={loading}>
              {c}
            </button>
          ))}
        </div>
        ...
        <div className="chip-group">
          {MOOD_CHIPS.map((c) => (
            <button key={c} className="chip chip-mood" onClick={() => chipSend(c)} disabled={loading}>
              {c}
            </button>
          ))}
        </div>
        ...
        <div className="chat-messages">
          {messages.length === 0 && (
            <div className="chat-empty">
              ...
              <p>Describe your vibe, drop an artist name, or pick a genre from the side. AI will build your sound blueprint.</p>
            </div>
          )}
          {messages.map((m, i) => (
